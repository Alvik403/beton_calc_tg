...
- **Конфигуратор** — одна базовая конфигурация на направление (материалы, рецепты, цены); выгрузка и вставка JSON
- **Калькулятор материалов** — под загрузкой Excel: выбор позиции и количества (м³ или шт), расчёт расхода материалов в кг
...
│   ├── web_profiles.json     # Сохранённая базовая конфигурация (бетон)
│   └── web_profiles_jbi.json # Сохранённая базовая конфигурация (ЖБИ)
├── tests/
├── docker-compose.yml
├── requirements.txt
...
2. Материалы, рецепты, цены → из YAML или единственной базовой веб-конфигурации
3. Расчёт:
...
| POST  | `/upload` | Загрузка Excel (file, scope, mode) |
...
| GET   | `/api/material-calculator/recipes?scope=` | Список позиций для калькулятора материалов |
| GET   | `/api/material-calculator?scope=&recipe=&quantity=` | Расход материалов (кг) на заданное количество |
| GET   | `/api/config?scope=` | Материалы, рецепты, цены (требует X-Config-Password) |
| POST  | `/api/config/profile` | Сохранить базовую веб-конфигурацию |
